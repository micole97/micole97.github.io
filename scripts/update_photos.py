...
after dropping new photos into a "Drop Zone" folder and/or reordering
existing photos in Import/Portfolio/.

For each category (Home/People/Places/Things):
  1. Look in Import/Portfolio/<Category>/Drop Zone/ for new .jpg files.
  ...
  3. Move each one into Import/Portfolio/<Category>/, renumbered to
     ...
     Files are MOVED, not copied — Drop Zone ends up empty again.
...
def process_category(category: str) -> int:
    ...
    drop_zone_dir = category_dir / DROP_ZONE_NAME
    category_dir.mkdir(parents=True, exist_ok=True)
    drop_zone_dir.mkdir(parents=True, exist_ok=True)

    all_dropped_files = [f for f in drop_zone_dir.iterdir() if f.is_file() and not f.name.startswith(".")]
    pending = sorted(
        (f for f in all_dropped_files if f.suffix.lower() in PHOTO_EXTS),
        key=import_sort_key,
    )

    for f in all_dropped_files:
        if f.suffix.lower() not in PHOTO_EXTS:
            print(f"  NOTE: {category}/{DROP_ZONE_NAME}/{f.name} isn't a .jpg file — leaving it there, not imported")
    ...
    for f in pending:
        ...
        if dest.exists():
            print(f"  SKIPPED {category}/{DROP_ZONE_NAME}/{f.name}: {dest_name} already exists — check manually")
            continue
        f.rename(dest)
        print(f"  {category}/{DROP_ZONE_NAME}/{f.name}  ->  {category}/{dest_name}")
        next_n += 1
        moved += 1
...
def main():
    ...
    print(f"Checking '{DROP_ZONE_NAME}' folders...")
    total = 0
    for category in CATEGORIES:
        total += process_category(category)

    if total == 0:
        print(f"No new photos found in any '{DROP_ZONE_NAME}' folder.")
    else:
